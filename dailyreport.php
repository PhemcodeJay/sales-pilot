...
// Update DailySalesQty in inventory table
$updateStmt = $connection->prepare('
    UPDATE sales_pilot.inventory inv
    SET inv.DailySalesQty = (
        SELECT SUM(s.DailySalesQty)
        FROM sales_pilot.sales s
...
function fetchInventoryDataByDate($connection, $Username, $date)
{
    $stmt = $connection->prepare('
        SELECT inventory.Product, category.categoryname, inventory.InventoryQty, inventory.StockQty, inventory.SupplyQty, inventory.Date, inventory.DailySalesQty
        FROM inventory
...
function fetchAndCalculateTotals($connection, $Username) {
    $stmt = $connection->prepare('SELECT COUNT(*) AS TotalProduct, SUM(InventoryQty) AS TotalInventoryQty FROM inventory WHERE Username = ?');
...
$stmt = $connection->prepare('SELECT category.categoryname AS categoryname, GROUP_CONCAT(inventory.Product) AS ProductNames, SUM(inventory.DailyStockQty) AS DailyStockQty, SUM(inventory.InventoryQty) AS InventoryQty, SUM(inventory.StockQty) AS StockQty, SUM(inventory.SupplyQty) AS SupplyQty
FROM inventory
...
while ($row = $result->fetch_assoc()) {
    ...
    $data['DailyStockQty'][] = $row['DailyStockQty'];
...
<div id="htmlReportTable"></div>
    ...
    var reportContent = '<h2>Daily Report</h2>';
    reportContent += '<table>';
    reportContent += '<tr><th>Category</th><th>Product</th><th>Available Daily Stock</th><th>Inventory Qty</th><th>Stock Qty</th><th>Supply Qty</th></tr>';
    ...
    for (var i = 0; i < chartData.categoryname.length; i++) {
        ...
        reportContent += '<td>' + numberWithCommas(chartData.DailyStockQty[i]) + '</td>';
    ...
    document.getElementById('htmlReportTable').innerHTML = reportContent;
        ...
        function generatePDFReport() {
            ...
            for (var i = 0; i < chartData.categoryname.length; i++) {
                reportContent += chartData.DailyStockQty[i] + "\t" + chartData.InventoryQty[i] + "\t" + chartData.StockQty[i] + "\t" + chartData.SupplyQty[i] + "\n";
            }
...
<h1>Daily Inventory</h1>
        ...
        <tbody>
            <?php
            // Fetch and display data for each date
            $stmt_dates = $connection->prepare('SELECT DISTINCT Date FROM inventory WHERE Username = ?');
            $stmt_dates->bind_param('s', $Username);
            $stmt_dates->execute();
            ...
            while ($row_date = $result_dates->fetch_assoc()) {
                ...
                // Fetch and display data for the date
                $dailyData = fetchInventoryDataByDate($connection, $Username, $date);

                if (!empty($dailyData)) {
                    foreach ($dailyData as $rowDay) {
                        echo "<td>" . fetchCategoryName($connection, $rowDay['categoryname']) . "</td>";
                        echo "<td>{$rowDay['Product']}</td>";
                        echo "<td>{$rowDay['StockQty']}</td>";
                        echo "<td>{$rowDay['SupplyQty']}</td>";
                        echo "<td>{$rowDay['InventoryQty']}</td>";
    ...
    <div class="button-container">
        ...
        <a href="inventorynew.php"><button class="button">Inventory</button></a>
        <a href="sales-dashboard.php"><button class="button">Sales</button></a>
        <a href="weeklyreport.php"><button class="button">Weekly</a>
        <a href="monthlyreport.php"><button class="button">Monthly</a>
        <a href="yearlyreport.php"><button class="button">Yearly</a>
    </div>
</body>
</html>
