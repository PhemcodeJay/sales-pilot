...
$stmt = $connection->prepare('SELECT category.categoryname AS categoryname, 
    GROUP_CONCAT(inventory.Product) AS ProductNames, 
    SUM(inventory.TotalRevenue) AS TotalRevenue, 
    SUM(inventory.InventoryValue) AS TotalInventoryValue, 
    SUM(inventory.SupplyValue) AS TotalSupplyValue, 
    SUM(inventory.SupplyQty) AS TotalSupplyQty, 
    SUM(inventory.InventoryQty) AS TotalInventoryQty, 
    SUM(inventory.StockQty) AS TotalStockQty, 
    SUM(inventory.StockValue) AS TotalStockValue,
    SUM(inventory.YearlySalesQty) AS TotalYearlySalesQty,
    (SUM(InventoryQty) - SUM(YearlysalesQty)) AS AvailableStock
    FROM inventory
...
while ($row = $resultCategories->fetch_assoc()) {
    ...
    $data['TotalStockQty'][] = $row['TotalStockQty'];
    $data['TotalInventoryQty'][] = $row['TotalInventoryQty'];
    $data['TotalStockValue'][] = $row['TotalStockValue'];
    $data['TotalYearlySalesQty'][] = $row['TotalYearlySalesQty'];
    $data['TotalRevenue'][] = $row['TotalRevenue'];
    $data['TotalSupplyValue'][] = $row['TotalSupplyValue'];
    $data['TotalSupplyQty'][] = $row['TotalSupplyQty'];
    $data['TotalInventoryValue'][] = $row['TotalInventoryValue'];
}


// Function to fetch data from inventory table
function fetchSalesData($connection, $Username, $field, $alias)
{
    $result = $connection->query("SELECT $field, $alias FROM inventory WHERE Username = '$Username'");
...
fetchSalesData($connection, $Username, 'Month', 'InventoryQty');

// Fetch monthly revenue data
fetchSalesData($connection, $Username, 'Month', 'SupplyQty');

// Fetch monthly expenses data
fetchSalesData($connection, $Username, 'Month', 'StockQty');


// Fetch data for inventory metrics with category information
...
function fetchAndCalculateTotals($connection, $Username) {
    $stmt = $connection->prepare('SELECT COUNT(*) AS TotalProduct, 
                                          ...
                                          SUM(SupplyValue) AS TotalSupplyValue,
                                          SUM(StockValue) AS TotalStockValue,
                                          (SUM(InventoryQty) - SUM(YearlySalesQty)) AS AvailableStock,
                                          SUM(YearlySalesQty) AS TotalYearlySalesQty
                                   FROM inventory 
                                   WHERE Username = ?');
    $stmt->bind_param('s', $Username);
    $stmt->execute();
...
<body class="g-sidenav-show bg-gray-200">
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 bg-gradient-dark" id="sidenav-main">
    <div class="sidenav-header">
      ...
      <a class="navbar-brand m-0" href="http://localhost/WEB/inventory-dashboard.php" target="_blank">
        <img src="http://localhost/WEB/logonew.jpg" class="navbar-brand-img h-100" alt="main_logo">
    ...
    <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link text-white active bg-gradient-primary" href="http://localhost/WEB/product.php">
        ...
        <li class="nav-item">
          <a class="nav-link text-white" href="http://localhost/WEB/inventorynew.php">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
        ...
        <li class="nav-item">
          <a class="nav-link text-white" href="http://localhost/WEB/sales-dashboard.php">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
  ...
  <div class="container-fluid py-1 px-3">
    <nav aria-label="breadcrumb">
      ...
      <h6 class="font-weight-bolder mb-0">Inventory Dashboard</h6>
          ...
          <div class="card">
            <div class="card-header p-3 pt-2">
              ...
              <div class="text-end pt-1">
                ...
                <h4 class="mb-0">$<?php echo number_format($totalData['TotalRevenue']); ?></h4>
              </div>
            </div>
            <hr class="dark horizontal my-0">
            <div class="card-footer p-3">
              <p class="mb-0"><span class="text-success text-sm font-weight-bolder"></span>Total Sales Revenue</p>
            </div>
          </div>
        </div>
        <div class="col-xl-3 col-sm-6 mb-xl-0 mb-4">
          <div class="card">
            <div class="card-header p-3 pt-2">
              ...
              <div class="text-end pt-1">
                ...
                <h4 class="mb-0">$<?php echo number_format($totalData['TotalInventoryValue']); ?></h4>
              </div>
            </div>
            <hr class="dark horizontal my-0">
            <div class="card-footer p-3">
              <p class="mb-0"><span class="text-success text-sm font-weight-bolder"></span>Total Inventory Value</p>
            </div>
          </div>
        </div>
        <div class="col-xl-3 col-sm-6 mb-xl-0 mb-4">
          <div class="card">
            <div class="card-header p-3 pt-2">
              ...
              <div class="text-end pt-1">
                ...
                <h4 class="mb-0">$<?php echo number_format($totalData['TotalStockvalue']); ?></h4>
              </div>
            </div>
            <hr class="dark horizontal my-0">
            <div class="card-footer p-3">
              <p class="mb-0"><span class="text-danger text-sm font-weight-bolder"></span>Total Stock Value</p>
            </div>
          </div>
        </div>
        <div class="col-xl-3 col-sm-6">
          <div class="card">
            <div class="card-header p-3 pt-2">
              ...
              <div class="text-end pt-1">
                <p class="text-sm mb-0 text-capitalize">Supply value</p>
                <h4 class="mb-0">$<?php echo number_format($totalData['TotalSupplyValue']); ?></h4>
              </div>
            </div>
            <hr class="dark horizontal my-0">
            <div class="card-footer p-3">
              <p class="mb-0"><span class="text-success text-sm font-weight-bolder"></span>Total Supply Value</p>
            </div>
          </div>
        </div>
      </div>
      <div class="row mt-4">
        <div class="col-lg-4 col-md-6 mt-4 mb-4">
          <div class="card z-index-2 ">
            ...
            <div class="card-body">
              ...
              <p class="text-sm ">Monthly Inventory Quantity</p>
              <hr class="dark horizontal">
              <div class="d-flex ">
                <i class="material-icons text-sm my-auto me-1">schedule</i>
                <p class="mb-0 text-sm">Monthly Inventory Quantity for all Products</p>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-md-6 mt-4 mb-4">
          <div class="card z-index-2  ">
            ...
            <div class="card-body">
              <h6 class="mb-0 ">Supply Quantity</h6>
              <p class="text-sm "><span class="font-weight-bolder"></span>Monthly Supply Quantity</p>
              <hr class="dark horizontal">
              <div class="d-flex ">
                <i class="material-icons text-sm my-auto me-1">schedule</i>
                <p class="mb-0 text-sm">Monthly Supply Quantity for all products</p>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-4 mt-4 mb-3">
          <div class="card z-index-2 ">
            ...
            <div class="card-body">
              ...
              <p class="text-sm ">Monthly Stock Quantity</p>
              <hr class="dark horizontal">
              <div class="d-flex ">
                <i class="material-icons text-sm my-auto me-1">schedule</i>
                <p class="mb-0 text-sm">Monthly Stock Quantity for all Products</p>
              </div>
            </div>
          </div>
        </div>
                ...
                <div class="row">
                    <div class="col-lg-6 col-7">
                        ...
                        <p class="text-sm mb-0">
                            ...
                            <span class="font-weight-bold ms-1">Inventory by Category</span>
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <table class="table align-items-center mb-0" id="expenditureTable">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-10 sortable" data-sort="stcok-qty">Stock Qty</th>
                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-10 sortable" data-sort="supply-qty">Supply Qty</th>
                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-10 sortable" data-sort="inventory-qty">Inventory Qty</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            ...
                            foreach ($data['categoryname'] as $index => $categoryname) {
                                ...
                                echo '<td class="text-center">' . number_format($data['TotalStockQty'][$index]) . '</td>';
                                echo '<td class="text-center">' . number_format($data['TotalSupplyQty'][$index]) . '</td>';
                                echo '<td class="text-center">' . number_format($data['TotalInventoryQty'][$index]) . '</td>';
                                echo '</tr>';
                            }
                            ?>
                        </tbody>
  ...
  <div class="card h-200">
    <div class="card-header pb-0">
      ...
      <p class="text-sm">
        ...
        <span class="font-weight-bold">Total Inventory Values by Category</span>
      </p>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
           <?php
                    ...
                    $categoryRevenue = array_combine($data['categoryname'], $data['TotalInventoryValue']);
                    ...
                    foreach ($categoryRevenue as $categoryname => $totalInventoryValue) {
                        echo '<tr>';
                        echo '<td>' . fetchCategoryName($connection, $categoryname) . '</td>';
                        echo '<td class="text-center">$' . number_format($totalInventoryValue) . '</td>';
                        echo '</tr>';
                    }
            ?>
        </tbody>
    ...
    new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          data: chartData.TotalInventoryQty,
          maxBarThickness: 6
        }, ],
      },
      options: {
