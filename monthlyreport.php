...
$query = '
    ...
    SUM(inventory.MonthlyStockQty) AS MonthlyStockQty, SUM(inventory.InventoryQty) AS InventoryQty,
...
// Fetch user records based on their Username and group by category
$stmt = $connection->prepare('SELECT category.categoryname AS categoryname, GROUP_CONCAT(inventory.Product) AS ProductNames, SUM(inventory.MonthlyStockQty) AS MonthlyStockQty, AVG(SalesPrice) AS AverageSalesPrice, SUM(inventory.InventoryQty) AS InventoryQty, SUM(inventory.StockQty) AS StockQty, SUM(inventory.SupplyQty) AS SupplyQty
FROM inventory
INNER JOIN category ON inventory.category_id = category.id_category
WHERE inventory.Username = ?
...
while ($row = $result->fetch_assoc()) {
    ...
    $data['MonthlyStockQty'][] = $row['MonthlyStockQty'];
        ...
        data: {
          ...
          "Kids"
            ...
            datasets: [
                ...
                {
    label: 'Monthly Stock',
    data: chartData.MonthlyStockQty,
...
<div id="htmlReportTable"></div>
    ...
    var reportContent = '<h2>Monthly Report</h2>';
    reportContent += '<table>';
    reportContent += '<tr><th>Category</th><th>Product</th><th>Available Monthly Stock</th><th>Inventory Qty</th><th>Stock Qty</th><th>Supply Qty</th></tr>';
    ...
    for (var i = 0; i < chartData.categoryname.length; i++) {
        ...
        reportContent += '<td>' + numberWithCommas(chartData.MonthlyStockQty[i]) + '</td>';
